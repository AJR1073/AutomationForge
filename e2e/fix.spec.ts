...
test.describe('/fix paste-to-fix', () => {
  test('loads with textarea and example loaders', async ({ page }) => {
    await page.goto('/fix');
    await expect(page.locator('h1')).toContainText('Fix your automation code');
  ...
  test('detects Shelly platform from pasted JS', async ({ page }) => {
    ...
    await expect(page.getByText('Detected: Shelly JS')).toBeVisible();
  });

  test('detects HA YAML platform', async ({ page }) => {
    await page.goto('/fix');
    await page.fill('#fix-input', 'automation:\n  - alias: "Test"\n    trigger:\n      - platform: state\n        entity_id: binary_sensor.test');
    await expect(page.getByText('Detected: Home Assistant YAML')).toBeVisible();
  });

  test('validates empty code input', async ({ page }) => {
    await page.goto('/fix');
    await expect(page.locator('#fix-btn')).toBeDisabled();
  });
  ...
  test('fixes broken HA YAML with tab indentation', async ({ page }) => {
    ...
    // Wait for results
    await expect(page.getByText('After')).toBeVisible({ timeout: 10000 });
    await expect(page.getByText(/Replaced tab indentation/)).toBeVisible();
  });

  test('loads Shelly example and fixes it', async ({ page }) => {
    await page.goto('/fix');
    await page.getByRole('button', { name: 'Shelly JS' }).click();
    await expect(page.locator('#fix-input')).not.toBeEmpty();
    
    await page.locator('#fix-btn').click();
    await expect(page.getByText(/After|Your Code \(no changes needed\)/)).toBeVisible({ timeout: 10000 });
  });

  test('copy button on fixed code', async ({ page }) => {
    await page.goto('/fix');
    await page.getByRole('button', { name: 'Home Assistant YAML' }).click();
    await page.locator('#fix-btn').click();
    await expect(page.getByText(/After|Your Code \(no changes needed\)/)).toBeVisible({ timeout: 10000 });

    const copyBtn = page.locator('#copy-fixed-ha');
    await copyBtn.click();
